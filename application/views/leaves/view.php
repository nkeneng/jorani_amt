<?php
/**
 * This view allows users to view a leave request
 * in read-only mode
    ...
    <div class="span6">
        ...
        <div class="row-fluid">
            <div class="span12">
                ...
                <input type="text"
                       name="startdate"
                       value="<?php $date = new DateTime($leave['startdate']);
                       echo $date->format(lang('global_date_format')); ?>"
                       readonly/>
                <select name="startdatetype"
                        style="<?php echo $leave['free_day'] ? 'display: none;' : ''; ?>"
                        readonly>
                ...
                <input type="text" name="enddate"
                       value="<?php $date = new DateTime($leave['enddate']);
                       echo $date->format(lang('global_date_format')); ?>"
                       readonly/>
                ...
                <label style="<?php echo $leave['free_day'] ? 'display: none;' : ''; ?>"
                       for="duration"><?php echo lang('leaves_view_field_duration'); ?></label>
                <input type="text" name="duration"
                       style="<?php echo $leave['free_day'] ? 'display: none;' : ''; ?>"
                       value="<?php echo $leave['duration']; ?>"
                       readonly/>
                ...
                <label style="<?php echo $leave['free_day'] ? 'display: none;' : ''; ?>"
                       for="cause"><?php echo lang('leaves_view_field_cause'); ?></label>
                <textarea
                        style="<?php echo $leave['free_day'] ? 'display: none;' : ''; ?>"
                        name="cause"
                        readonly><?php echo $leave['cause']; ?></textarea>
                ...
                <?php if ($leave['status'] == LMS_PLANNED) { ?>
                    <a href="<?php echo base_url(); ?>leaves/request/<?php echo $leave['id'] ?>/"
                ...
                <?php if ($leave['status'] == LMS_ACCEPTED) { ?>
                    <a href="<?php echo base_url(); ?>leaves/cancellation/<?php echo $leave['id'] ?>"
                       class="btn btn-primary"><i
                                class="mdi mdi-undo"></i>&nbsp;<?php echo lang('Cancellation'); ?>
                    </a>
                    <br/><br/>
                <?php } ?>
                <?php if ($leave['status'] == LMS_REQUESTED) { ?>
                    <a href="<?php echo base_url(); ?>leaves/reminder/<?php echo $leave['id']; ?>"
                       title="<?php echo lang('leaves_button_send_reminder'); ?>"
                       class="btn btn-primary"><i
                                class="mdi mdi-email"></i>&nbsp;<?php echo lang('leaves_button_send_reminder'); ?>
                    </a>
                    <br/><br/>
                <?php } ?>
                ...
                <?php if (($leave['status'] == LMS_PLANNED) || ($is_hr)) { ?>
                    <a href="<?php echo base_url(); ?>leaves/edit/<?php echo $leave['id'] ?>"
                       class="btn btn-primary"><i
                                class="mdi mdi-pencil"></i>&nbsp;<?php echo lang('leaves_view_button_edit'); ?>
                    </a>
                    &nbsp;
                <?php } ?>
