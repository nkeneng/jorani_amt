<?php
/**
 * This partial view show the history of changes
 * on a leave request
...
<div class="row-fluid">
    <div class="span12">
        ...
        <table class="table" id="leaveshistory"
               width="100%">
            <thead>
            <tr>
            ...
            foreach ($events as $event) {
                ...
                $startDate = $event['free_day'] ? $date->format(lang('global_date_format')) : $date->format(lang('global_date_format')) . ' (' . lang($event['startdatetype']) . ')';
                $objLeave->startDate = $startDate;
                $date = new DateTime($event['enddate']);
                $endDate = $event['free_day'] ? $date->format(lang('global_date_format')) : $date->format(lang('global_date_format')) . ' (' . lang($event['enddatetype']) . ')';
                $objLeave->endDate = $endDate;
                $freeDay = $event['free_day'] ? lang($event['free_day']) : 'x';
                $objLeave->free_day = $freeDay;
                ...
                $objLeave->free_day = ($freeDay == $lastObject->free_day) ? '' : $freeDay;
...
if (!empty($leave)) {
    ...
    $objLeave->startDate = $event['free_day'] ? $date->format(lang('global_date_format')) : $date->format(lang('global_date_format')) . ' (' . lang($leave['startdatetype']) . ')';
    $date = new DateTime($leave['enddate']);
    $objLeave->endDate = $event['free_day'] ? $date->format(lang('global_date_format')) : $date->format(lang('global_date_format')) . ' (' . lang($leave['enddatetype']) . ')';
    $objLeave->cause = $leave['cause'];
    $objLeave->free_day = $leave['free_day'] ? lang($leave['free_day']) : 'x';
    ...
    array_push($history, $objLeave);
            ...
            <?php foreach ($history as $objLeave): ?>
                ...
                <tr>
                    <td class="muted"><i
                                class="<?php echo $objLeave->typeIcon; ?>"></i>
                    </td>
                    ...
                    <td style="text-align: center;"><?php echo $objLeave->free_day; ?></td>
