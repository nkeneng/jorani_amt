...
<div class="row">
    ...
    function verifStatus($array)
    {
        for ($i = 0; $i < count($array); $i++) {
            if ($i > 0) {
                if (($array[$i - 1]['status'] != $array[$i]['status']) && $array[$i - 1]['status'] != 2 && $array[$i]['status'] != 2) {
                    return true;
                }
            }
        }
    ...
    <div class="span8">
        <span class="label"><input
                    type="checkbox" <?php echo $checked; ?> id="chkPlanned"
                    class="filterStatus" <?php echo $disable; ?>> &nbsp;<?php echo lang('Planned'); ?></span>
        &nbsp;
        <span class="label label-success"><input
                    type="checkbox" <?php echo $checked; ?> id="chkAccepted"
        ...
        <span class="label label-important"
              style="background-color: #ff0000;"><input
                    type="checkbox" <?php echo $checked; ?> id="chkRejected"
                    class="filterStatus" <?php echo $disable; ?>> &nbsp;<?php echo lang('Rejected'); ?></span>
        &nbsp;
        <span class="label label-important"
              style="background-color: #ff0000;"><input
                    type="checkbox" checked
        ...
        <span class="label label-important"
              style="background-color: #ff0000;"><input
                    type="checkbox" <?php echo $checked; ?> id="chkCanceled"
...
<div class="">
    ...
    <div class=""
         ...
         id="btnAction">
        <button type="button" id="btnAccepted"
                class="btn btn-success"
                style="margin-right: 20px;"><?php echo lang('Accepted'); ?></button>
        <button type="button" id="btnRejected"
                class="btn btn-danger"><?php echo lang('Rejected'); ?></button>
    </div>
</div>
...
<table cellpadding="0" cellspacing="0" border="0"
    ...
    <tr>
        <th>
            <input title="<?php echo lang('check_all'); ?>"
                   type="checkbox"
    ...
    <?php foreach ($requests as $request):
        ...
        ?>
        <?php if (!$request['parent_leave_id'] && $showAll) { ?>
        <?php if (!$request['parent_leave_id']) : ?>
            <tr style="<?php echo $request['free_day'] ? 'background:#D9FCD8' : ''; ?>">
                <td></td>
                <td style="display:none"><?php echo $request['leave_id']; ?></td>
                <td data-order="<?php echo $request['leave_id']; ?>">
                    ...
                    <div class="pull-right">
                        ...
                        <?php if ($this->config->item('enable_history') === TRUE) { ?>
                            &nbsp;
                            <a href="<?php echo base_url(); ?><?php if ($request['parent_leave'] == 1) : ?>requests/details/<?php echo $request['leave_id']; ?>/true<?php else : ?>leaves/leaves/<?php echo $request['leave_id']; ?><?php endif; ?>"
                ...
                <?php
                if ($request['parent_leave'] == 1) {
                    if ($request['sub_leaves_treated'] == 1) {
                        echo "<td><span class='label label-info'>" . lang('treated_ok') . "</span></td>";
                    } else {
                        if (array_filt($requests, $request['id'])) {
                            echo "<td><span class='label label-info'>" . lang('treated_ok') . "</span></td>";
                        } else {
        ...
        <?php endif; ?>
    <?php } else { ?>
        <?php if (!$request['parent_leave_id'] && $request['sub_leaves_treated'] == 0) : ?>
            <tr style="<?php echo $request['free_day'] ? 'background:#D9FCD8' : ''; ?>">
                <td></td>
                <td style="display:none"><?php echo $request['leave_id']; ?></td>
                <td data-order="<?php echo $request['leave_id']; ?>">
                    ...
                    <div class="pull-right">
                        ...
                        <?php if ($this->config->item('enable_history') === TRUE) { ?>
                            &nbsp;
                            <a href="<?php echo base_url(); ?><?php if ($request['parent_leave'] == 1) : ?>requests/details/<?php echo $request['leave_id']; ?><?php else : ?>leaves/leaves/<?php echo $request['leave_id']; ?><?php endif; ?>"
                               title="<?php echo lang('requests_index_thead_tip_view'); ?>"><i
                                        class="mdi mdi-eye nolink"></i></a>
                            &nbsp;
                            <a href="#"
...
<div class="row-fluid">
    <div class="span12">
        <a href="<?php echo base_url(); ?>requests/export/<?php echo $filter; ?>"
           class="btn btn-primary"><i
                    class="mdi mdi-download"></i>&nbsp; <?php echo lang('requests_index_button_export'); ?>
        </a>
        &nbsp;&nbsp;
        <a href="<?php echo base_url(); ?>requests/all"
           class="btn btn-primary"><i
...
<script type="text/javascript">
    ...
    function filterStatusColumn() {
        ...
        leaveTable.columns(9).search(filter, true, false).draw();
    }
    ...
    $(document).ready(function () {
        ...
        leaveTable = $('#leaves').DataTable({
            'columnDefs': [{
                ...
                'render': function (data, type, full, meta) {
                    return `<input type="checkbox" id="${full[1]}" name="id[]" value="' + $('<div/>').text(data).html() + '">`;
                }
            }],
            order: [[2, "desc"]],
            language: {
        ...
        $('#example-select-all').on('click', function () {
            if (this.checked) {
                new Toast({
                    message: '<?php echo lang('you_have_selected_all_the_elements_of_the_table');?>',
                    type: 'warning'
                });
                tabId = JSON.parse('<?php echo json_encode($requests);?>')
                tabId = tabId.map(x => x?.id)
                $('#btnAction').show()
                $('#selectItem').text(tabId.length + ' ' + '<?php echo lang('selected');?>')
            ...
            var rows = leaveTable.rows({'search': 'applied'}).nodes();
            // Check/uncheck checkboxes for all rows in the table
            $('input[type="checkbox"]', rows).prop('checked', this.checked);
        });
        // Handle click on checkbox to set state of "Select all" control
        $('#leaves tbody').on('change', 'input[type="checkbox"]', function () {
    ...
    <?php } ?>
        ...
        $('#cboLeaveType').on('change', function () {
            ...
            if (leaveType != '') {
                leaveTable.columns(8).search("^" + leaveType + "$", true, false).draw();
            } else {
                leaveTable.columns(8).search("", true, false).draw();
            }
        });
    ...
    $(function () {
        $("#leaves tbody input[type=checkbox]").change(function () {

            if ($(this).is(":checked")) {
                tabId.push($(this).attr("id"))
            } else {
                tabId = tabId.filter(x => x != $(this).attr("id"))
            }
            if (tabId.length > 0) {
                $('#btnAction').show()
                if (tabId.length == 1) {
                    ...
                    $('#selectItem').text(tabId.length + ' ' + '<?php echo lang('selecteds');?>')
                }
            } else {
                $('#btnAction').hide()
                $('#selectItem').text('')
        ...
        $('#btnAccepted').on('click', function (event) {
            if (tabId.length > 0) {
                window.location.href = `requests/listAccepted?array=${tabId}`
            } else {
                new Toast({
                    message: '<?php echo lang('no_items_selected');?>',
                    type: 'danger'
                });
            }
        })
        $('#btnRejected').on('click', function (event) {
            if (tabId.length > 0) {
                window.location.href = `requests/listRejected?array=${tabId}`
            } else {
                new Toast({
                    message: '<?php echo lang('no_items_selected');?>',
                    type: 'danger'
